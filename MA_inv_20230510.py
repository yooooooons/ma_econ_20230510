...
LIST_target = [BTC_0_dic, ETH_1_dic, ETC_5_dic, WAVES_7_dic, ARK_14_dic, BTG_21_dic]
...
def buy_sell_normal (dic_target) :
    ...
    if dic_target['bought_state'] == 1 :   # 매수가 되어 있는 상태라면
        ...
        if (DF_check['volume'][-2] < (dic_target['sell_method_vol_cri'] * DF_check.iloc[(-5) : -2]['volume'].mean())) and (DF_check['ratio_ema_long'][-1] < (dic_target['ratio_sell'] + dic_target['add_ratio_sell'])) :
            transaction_sell = upbit.sell_market_order(coin_inv, get_balance(coin_inv[4:]))   # 시장가에 매도
            time.sleep(5)
            print ('\nnow :', (datetime.datetime.now() + datetime.timedelta(seconds = (time_factor * 3600))))
        ...
        if (DF_check['volume'][-2] < (dic_target['sell_method_vol_cri'] * DF_check.iloc[-5 : -2]['volume'].mean())) and ((DF_check.loc[DF_check.iloc[-6 : -1]['ema_mid'].idxmax()]['ema_mid'] - DF_check['ema_mid'][-1]) > (dic_target['ratio_diff_ema_mid'] * DF_check['open'][-1])) :
            transaction_sell = upbit.sell_market_order(coin_inv, get_balance(coin_inv[4:]))   # 시장가에 매도
            time.sleep(5)
            print ('\nnow :', (datetime.datetime.now() + datetime.timedelta(seconds = (time_factor * 3600))))
...
while True:
    ...
    if (now.minute % time_unit == 1) & (0 < (now.second % 60) <= 59) :   # N시:01:00초 ~ N시:01:15초 사이 시각이면
